const  { GUILDID, JOINTOCREATECHANNEL } = (process.env);
const jointocreatemap = new Map();
module.exports = function (chitanda) {
    const description = {
        name: "jointocreate",
  ...
  new Promise(resolve => {
    setInterval(() => {
      resolve(2);
        try{
          const guild = chitanda.guilds.cache.get(GUILDID);
...
//voice state update event to check joining/leaving channels
    chitanda.on("voiceStateUpdate", (oldState, newState) => {
